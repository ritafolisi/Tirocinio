...
\begin{document}
	...
	\`E stato inoltre necessario, come per il precedente algoritmo, introdurre una funzione che calcolasse l'RMSE. Non è stato però possibile ricavare dal codice il valore delle membership predette, e quindi il valore dell'RMSE è stato calcolato sulla base della label predetta; per lo stesso motivo non è stato possibile realizzare grafici con curve di livello o heatmap. L'algoritmo presenta un tempo di esecuzione notevole, dovuto solo in parte alla fase di model selection; sono stati fatti esperimenti sui tre dataset ottenuti da Iris e sul dataset Breast Cancer. 
	\section{Risultati}
